...
|------|-------------|---------|----------|
| **Agent 数量** | 1 个统一 Agent | 8 个核心 Agent | Sprint 13 将 22 个合并为 8 个（architect/decision-maker/developer/deployer/planner/analyst/reviewer/chat-judge），通过 execution mode 实现角色切换。仍多于 Claude Code 但信息损耗已大幅降低 |
...
**核心问题总结**（Sprint 13 合并后部分缓解）：RebuilD 用"多 Agent 协作"解决复杂性，Sprint 13 将 22 个 Agent 合并为 8 个后情况改善：

1. **Agent 间通信损耗** — 每次 handoff 是一次有损压缩。Sprint 13 合并至 8 个 Agent + Blackboard 无损通信后，handoff 损耗已大幅降低
2. **调试困难** — 8 个 Agent 比 22 个更易追踪，但仍比单一 Agent 复杂。Execution Trace Dashboard（Sprint 8-9）提供可视化回溯
3. **成本倍增** — Sprint 6 Multi-Model 配置（6 个 agent 用 gpt-4o-mini）+ Sprint 13 Agent 合并已降低 40-60% 成本
4. **Architect 定位矛盾** — 仍存在。Architect 作为 ReAct Agent 依赖 LLM 编排子 Agent，但 Sprint 13 减少层级后不确定性已降低
...
| # | 建议 | 状态 | 说明 |
|---|------|------|------|
| B1 | 合并 22 个 Agent → 5-7 个 | ✅ 已完成 | Sprint 13（`5ef7fca`）将 20+ agent 合并为 8 个核心 agent（architect/decision-maker/developer/deployer/planner/analyst/reviewer/chat-judge），通过 execution mode 实现角色切换。Sprint 13 supplement（`3803751`）将全部 8 个 prompt 升级为 agency-agents 完整风格（8 板块） |
...
| # | 建议 | 状态 | 说明 |
|---|------|------|------|
| D1 | 测试覆盖 | ✅ 已改善 | Sprint 10（`f4afed3`）新增 meta-pipeline 集成测试（mock LLM pool），Sprint 11（`46a1cf9`）新增 team-coordinator 9 个单测，Sprint 12（`215ad96`）新增 tool-approval-audit 5 个单测，Sprint 13（`5ef7fca`）新增 agent 合并相关测试。共 129 tests/10 suites |
...
## 七、当前状态诊断（更新版 2026-03-09 Sprint 15 后）

| 架构层 | 完成度 | 状态 |
|--------|--------|------|
| 数据库 & Schema | 98% | 28 个 migration，结构完整（含 architect checkpoint + execution traces + team coordinator 索引 + tool approval audits） |
| Auth & RBAC | 85% | 中间件、API Key、角色体系已实现。Sprint 4 增加命令注入过滤、安全头、CORS |
| Agent 核心引擎 | 99% | BaseAgent、工具系统、LLM Pool、ContextBudget、Blackboard（含 TTL/容量生命周期）、onCheckpoint 回调、Multi-Model 配置、全链路错误日志（无 silent catch）。8 个核心 Agent 全部升级为 agency-agents 完整 prompt 风格 |
| Agent 架构 | 95% | Sprint 13 将 20+ agent 合并为 8 个核心 agent，每个 agent 通过 execution mode 实现多角色切换（如 analyst: research/advocate/critique/arbitrate/retrieve）。Prompt 统一为 Identity & Memory / Core Mission / Critical Rules / Workflow Process / Deliverables / Communication Style / Success Metrics / Advanced Capabilities 8 板块结构 |
| Meta Pipeline (DM→AC) | 99% | Zod 校验、DM checkpoint、Architect 工具审批、StructuredRequirements 注入、DM↔Architect Blackboard 无损通信、Architect checkpoint & resume、子 Agent Blackboard 透传、Webhook 事件桥接（pipeline/DM/Architect 生命周期推送飞书/钉钉/Slack/Custom）、dm.run() try-catch 防崩溃均已实现 |
| Chat-First 架构 | 90% | L1/L2/L3 路由完整，L3 全链路含 Blackboard hydrate + 兜底种子 + 生命周期管理。ChatEngine 通过 TeamCoordinator 管理 agent 生命周期。Sprint 15 修复 SSE stuck-state：stream timeout 安全网、done 事件防御、panel 全量 reset |
| Team Coordinator | 90% | Sprint 11 修复三大问题：agent status 真实追踪、task dependency 含 blocked_by 检查、mailbox 三级清理。API routes 全部委托 coordinator |
| 前端 UI | 92% | 聊天、项目、看板、信号、PlanPanel、ClarificationForm、DMDecisionPanel、ToolApprovalPanel、ArchitectResumePanel、成本面板、Execution Trace Dashboard、Webhook 配置面板已就位。Sprint 15 修复 DMDecisionPanel/ArchitectResumePanel SSE handler 补全（tool_approval/architect_failed 事件）、PlanPanel 乐观状态 bug、ClarificationForm 错误恢复 |
| 信号采集 | 70% | 多平台适配器存在，cron 配置完成 |
| 信号→执行链路 | 87% | Sprint 2 打通一键 pipeline，Sprint 3 无损上下文传递，Sprint 4 checkpoint 断点恢复，Sprint 5 Blackboard 生命周期 |
| 端到端流程 | 90% | L1/L2 端到端可用；L3 全链路连通含 Blackboard + checkpoint + 子 Agent 共享状态 + Webhook 通知；信号→执行一键可用；成本可见；执行轨迹可回溯；前端 stuck-state bug 已修复 |
| 测试覆盖 | 90% | 129 tests/10 suites，含 meta-pipeline 集成测试 + team-coordinator 单测 + tool-approval-audit 单测 + agent 合并测试。核心链路已有测试保障 |
| 成本优化 | 85% | Sprint 6 Multi-Model 配置 + Sprint 13 Agent 合并（8 个 agent 减少 handoff 和 LLM 调用次数），预估总计降低 50-70% LLM 调用成本 |

**核心矛盾更新**：三套并行架构已统一为 Chat-First 单一入口。安全加固（S1-S4）、Agent 合并（B1/A5 22→8）、Blackboard 通信（B2+A1+A2）、可观测性（O1+O2+O3+O4 全部清零）、技术债（T1+T2+T4+T6）、Team Coordinator（A3）、Tool Approval 审计日志（A4）、Webhook 通知桥接（O3）、前端 SSE 稳健性修复均已完成。当前主要矛盾缩减为：(1) ChatEngine L2/L3 端到端实战验证不足（需要在真实 LLM 调用下跑通完整流程）；(2) Vercel 长任务超时未解决（T3）。
...
| A3 | Team Coordinator 完善 | ✅ 已完成（Sprint 11, `46a1cf9`）— agent status 存入 config JSONB（不再硬编码 idle）；updateTaskStatus 含 blocked_by 依赖检查；setTaskDependencies 双向维护；markAsRead/cleanupMailbox/clearMailbox 三级清理；disbandTeam 清空 mailbox；ChatEngine 通过 coordinator 管理 architect 生命周期 | `lib/services/team-coordinator.ts` + API routes |
| A4 | Tool Approval 审计日志 | ✅ 已完成（Sprint 12, `215ad96`）— `tool_approval_audits` 表持久化审批事件（requested/approved/rejected/timed_out），区分 user 主动拒绝与 timeout 自动拒绝，支持 rejection_reason，GET `/api/conversations/[id]/approvals` 查询历史，5 个单测 | `lib/services/tool-approval.ts` + `tool-approval-audit.ts` |
| A5 | Agent 合并 22→8 | ✅ 已完成（Sprint 13, `5ef7fca` + `3803751`）— 合并为 8 核心 agent：architect/decision-maker/developer/deployer/planner/analyst/reviewer/chat-judge。每个 agent 通过 execution mode 多角色切换。Prompt 统一为 agency-agents 8 板块风格 | `agents/` 全目录重构 |
...
| O3 | Webhook 通知 | ✅ 已完成（Sprint 14 CRUD `ec67ae2` + Sprint 15 Bridge `ab818ed`）— WebhookService CRUD + 飞书/钉钉/Slack/Custom 格式化 + `emitWebhookEvent()` helper 桥接 pipeline 事件到 webhook 系统。9 种事件类型：pipeline_started/pipeline_complete/dm_decision_complete/architect_started/architect_complete/architect_failed/deploy_complete/deploy_failed/pr_created | `lib/services/webhook.ts` + `lib/services/chat-engine.ts` + `skills/meta-pipeline.ts` |
| O4 | Multi-Model per Agent | ✅ 已完成（Sprint 6, `1822ecb` + Sprint 13 更新）— `agents/config.json` 配置合并后的低复杂度 agent（planner/analyst/chat-judge）使用 gpt-4o-mini，核心 agent（architect/decision-maker/developer/deployer/reviewer）保持 gpt-4o | `agents/config.json` |
...
| T2 | Pipeline 集成测试 | ✅ 已完成（Sprint 10, `f4afed3`）— mock LLM pool，覆盖 DM→Architect 链路 + 错误恢复 + Blackboard 集成，81 tests/8 suites 全过 | `lib/core/__tests__/meta-pipeline.integration.test.ts` |
...
- **T2 — Pipeline 集成测试**（commit `f4afed3`）：mock LLM pool 层，端到端验证 DM→Architect 链路。新增 `meta-pipeline.integration.test.ts`，覆盖 DM PROCEED/HALT、Architect 成功/失败、Blackboard 集成。测试总数从 64 增至 81

### Sprint 11 — ✅ 已完成

- **A3 — Team Coordinator 完善**（commit `46a1cf9`）：修复三大问题
  - Agent status 不再硬编码 idle — 存储在 `config.agent_statuses` JSONB，ChatEngine 通过 `updateAgentStatus()` 追踪 architect 生命周期（working→completed/failed）
  - Task dependency 检查 — `updateTaskStatus()` 在 pending→in_progress 时验证 blocked_by 全部 completed；`setTaskDependencies()` 双向维护 blocks/blocked_by
  - Mailbox 清理 — `markAsRead()`、`cleanupMailbox(24h)`、`clearMailbox()`；`disbandTeam()` 自动清空 mailbox
  - API routes 全部通过 coordinator（tasks PATCH、mailbox PATCH/DELETE、team DELETE、intervene POST）
  - ChatEngine 移除 `createTeam()` 私有方法，使用 `teamCoordinator.formTeam()`
  - 9 个单元测试全部通过

### Sprint 12 — ✅ 已完成

- **A4 — Tool Approval 审计日志**（commit `215ad96`）：
  - 新建 `tool_approval_audits` 表（migration 028），持久化 requested/approved/rejected/timed_out 四种状态
  - `recordToolApprovalEvent()` fire-and-forget：status=requested 时 insert，其他状态 update（匹配 approval_id）
  - `listToolApprovalAudits()` 按 created_at DESC 查询历史
  - `ToolApprovalService` 集成：timeout 自动记录 timed_out（decided_by='timeout'），resolve 记录 approved/rejected（decided_by='user'）+ rejection_reason
  - `ChatEngine` 集成：onApprovalRequired 回调中记录 requested 审计 + 传入 conversationId
  - API：reject_tool 支持 rejection_reason，新增 GET `/api/conversations/[id]/approvals` 历史查询
  - 5 个单测全部通过，86 tests/9 suites 无回归

### Sprint 13 — ✅ 已完成

- **A5/B1 — Agent 合并 22→8**（commit `5ef7fca`）：
  - 将 20+ agent 合并为 8 个核心 agent：architect、decision-maker、developer、deployer、planner（PM+Tech Lead+Orchestrator）、analyst（Researcher+Blue Team+Critic+Arbitrator+Knowledge Curator）、reviewer（QA+Code Reviewer+Supervisor）、chat-judge（renamed from complexity-assessor）
  - 每个 agent 通过 execution mode 实现多角色切换
  - `agents/config.json` Multi-Model 配置同步更新为新名称
  - 129 tests/10 suites 全部通过

### Sprint 13 supplement — ✅ 已完成

- **8 Agent Prompt 升级为 agency-agents 完整风格**（commit `3803751`）：
  - 2 个重写（architect.ts、decision-maker.ts）+ 2 个新建（developer/prompts/system.ts、deployer/prompts/system.ts）+ 4 个增强（planner、analyst、reviewer、chat-judge）
  - 每个 prompt 统一为 8 板块：Identity & Memory / Core Mission / Critical Rules / Workflow Process / Deliverables / Communication Style / Success Metrics / Advanced Capabilities
  - developer/deployer index.ts 添加 DEVELOPER_PROMPT/DEPLOYER_PROMPT fallback
  - Decision Maker prompt 中旧 agent 名称（researcher/blue-team/critic 等）更新为 analyst + modes

### Sprint 14 — ✅ 已完成

- **O3 — Webhook CRUD + ChatEngine exec mode 修复**（commit `ec67ae2`）：
  - `WebhookService`：飞书/钉钉/Slack/Custom 四种 provider 格式化，配置缓存 TTL=1min，`dispatch()` 按事件过滤匹配配置，fire-and-forget 分发
  - Webhook 配置 API：`GET/POST /api/settings/webhooks`（列表+创建），`PUT/DELETE /api/settings/webhooks/[id]`（更新+删除），`POST /api/settings/webhooks/test`（测试发送）
  - `WebhookService.init()` 订阅 messageBus `agent-log` 通道，过滤 `payload.webhook_event` 字段
  - 修复 ChatEngine exec mode P0 bug：`executePlan()` 未正确路由到 `handleAgentTeam()`
  - 清理死代码（`loadAgentFactory` 中过时 agent 引用）

### Sprint 15 — ✅ 已完成

- **Webhook Bridge + SSE Robustness + Panel State Hardening**（commit `ab818ed`）：

**Workstream A — Webhook 事件桥接（3 files）**：
  - `lib/services/webhook.ts`：扩展 `WebhookEventType` 新增 5 种事件（`pipeline_started`/`dm_decision_complete`/`architect_started`/`architect_complete`/`architect_failed`），共 9 种；新增 `emitWebhookEvent()` helper 发布到 `agent-log` 通道（`WebhookService` 已订阅该通道过滤 `webhook_event`）
  - `lib/services/chat-engine.ts`：4 处 `emitWebhookEvent` 调用（DM 决策完成、Architect 启动、Architect 成功、Architect 失败）
  - `skills/meta-pipeline.ts`：3 处 `emitWebhookEvent` 调用（pipeline 启动、DM 终止、pipeline 完成）；`dm.run()` 包裹 try-catch，失败时返回 HALT decision 而非崩溃

**Workstream B — 前端 SSE 稳健性（5 files）**：
  - `components/chat/ChatView.tsx`：新增 `done` 事件处理（防 silent drop）；新增 5 分钟 stream timeout 安全网（finally 中 clearTimeout）
  - `components/chat/DMDecisionPanel.tsx`：`handleSSEStream` 补全 `tool_approval_required`/`tool_approval_resolved`/`architect_failed` 3 种事件处理
  - `components/chat/ArchitectResumePanel.tsx`：同上 3 种事件处理
  - `components/chat/PlanPanel.tsx`：`approvePlan()` 从乐观调用改为 fetch 成功后调用（防止 API 失败时 UI 状态错误）
  - `components/chat/ClarificationForm.tsx`：catch 块增加 `console.error` 日志；错误时不隐藏表单，用户可重试

**Workstream C — Panel 状态加固（1 file）**：
  - `store/slices/chatSlice.ts`：`hideDmPanel`/`hideToolApproval`/`hideArchitectPanel` 从 spread+visible:false 改为全量 reset（清除 decision/approvalId/toolName/toolArgs/agentName/errorMessage 等字段，防止 stale state carryover）

**验证结果**：
  - TypeScript 编译：`npx tsc --noEmit` — 0 errors
  - 9 files modified, +197 / -31

### Sprint 16 候选方案

- **方案 A — Vercel 长任务改造 (T3)**：maxDuration=300 改为 Background Job + polling 模式
- **方案 B — ChatEngine L2/L3 实战打通**：确保端到端流程在真实 LLM 调用下可用，修复潜在集成问题
- **方案 C — 企业微信 Webhook Provider**：新增 WeCom（企业微信）作为第五种 webhook provider
...
> RebuilD 的底层引擎（BaseAgent、Tool Registry、LLM Pool、Blackboard）设计扎实。Sprint 13 完成 Agent 合并（22→8），顶层编排复杂度大幅降低。
>
> **2026-03-07 更新**：L1/L2/L3 路由系统已实现，三套并行架构统一为 Chat-First 单一入口。L1/L2 端到端可用，L3 链路已连通但 meta-pipeline 内部缺 checkpoint。
>
> **2026-03-08 更新**（Sprint 1-12）：
...
> - Sprint 5：Blackboard 生命周期管理 A1+A2（`24df22c`），TTL/容量淘汰 + 子 Agent Blackboard 透传
> - Sprint 6：成本面板 O2 + Multi-Model 配置 O4（`1822ecb`），用户可见 LLM 成本 + 6 个低复杂度 agent 降级至 gpt-4o-mini
> - Sprint 7：批量技术债清理 T1+T4+T6（`8cacc61`），26 处 silent catch 清理 + PM shim 删除 + PlanTask 类型安全
> - Sprint 8：Execution Trace 后端持久化 O1（`d030564`）
> - Sprint 9：Execution Trace 前端 Dashboard O1，Phase 2 可观测性全部清零
> - Sprint 10：Pipeline 集成测试 T2（`f4afed3`），81 tests/8 suites
> - Sprint 11：Team Coordinator 完善 A3（`46a1cf9`），9 个单测
> - Sprint 12：Tool Approval 审计日志 A4（`215ad96`），5 个单测，86 tests/9 suites
>
> **2026-03-09 更新**（Sprint 13-15）：
> - Sprint 13：Agent 合并 A5/B1（`5ef7fca`），20+ agent → 8 核心 agent（architect/decision-maker/developer/deployer/planner/analyst/reviewer/chat-judge），每个 agent 通过 execution mode 多角色切换，129 tests/10 suites
> - Sprint 13 supplement：8 Agent Prompt 全面升级为 agency-agents 风格（`3803751`），统一 8 板块结构（Identity & Memory / Core Mission / Critical Rules / Workflow Process / Deliverables / Communication Style / Success Metrics / Advanced Capabilities）
> - Sprint 14：Webhook CRUD 基础设施（`ec67ae2`）— WebhookService + 配置 API + 飞书/钉钉/Slack/Custom 四种 provider；修复 ChatEngine exec mode P0 bug
> - Sprint 15：Webhook Bridge + SSE Robustness + Panel State Hardening（`ab818ed`）— emitWebhookEvent helper 桥接 7 个 pipeline 生命周期点到 webhook 系统（9 种事件类型）；ChatView stream timeout + done 事件防御；DMDecisionPanel/ArchitectResumePanel SSE handler 补全；PlanPanel/ClarificationForm 错误恢复；chatSlice panel 全量 reset
>
> **阶段 A 全部完成（A1-A5 全部清零），阶段 B 全部核心项（B1 Agent 合并 + B2 Blackboard 通信）已完成，阶段 C 产品差异化链路（C2+C3）已打通，阶段 D 测试覆盖+错误处理+国际化（D1+D2+D3）已完成。安全加固 P0 全部完成，可观测性 O1+O2+O3+O4 全部清零，技术债 T1+T2+T4+T6 已清零。前端 SSE stuck-state 全面修复。系统架构层面已基本完善，进入产品化验证阶段。**
>
> **剩余待优化项（2 项）**：
> 1. Vercel 长任务改造（T3）— maxDuration=300 改为 Background Job + polling
