        ...
        <div className="relative pl-6">
          ...
          {chart.data.map((item, i) => (
            <div key={i} className="relative pb-4 last:pb-0">
              {/* Dot */}
              <div className="absolute left-[-17px] top-1.5 w-3 h-3 rounded-full border-2 border-blue-500 bg-[var(--bg-surface)]" />

              <div className="ml-1">
                <div className="text-xs font-medium text-blue-400">{item.period}</div>
                <div className="text-xs text-[var(--text-primary)] mt-0.5">{item.events}</div>
                {item.revenue && (
                  <div className="text-[11px] text-[var(--text-muted)] mt-0.5">{item.revenue}</div>
                )}
                {/* Render any extra fields */}
                {Object.entries(item)
                  .filter(([k]) => !["period", "events", "revenue"].includes(k))
                  .map(([k, v]) => (
                    <div key={k} className="text-[11px] text-[var(--text-muted)] mt-0.5">
                      {k}: {v}
                    </div>
                  ))}
              </div>
