    ...
    <div className="flex items-center gap-0.5">
      {Array.from({ length: max }, (_, i) => (
        <svg
          ...
          height="12"
          viewBox="0 0 12 12"
          className={i < score ? "text-amber-400" : "text-[var(--text-disabled)]"}
        >
          <path
    ...
    <div>
      ...
      <div className="px-4 pb-3 space-y-2.5">
        {chart.ratings.map((item, i) => (
          <div
            key={i}
            className="flex items-center gap-3 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-glass)] px-3 py-2"
          >
            <div className="min-w-0 flex-1">
              <div className="text-xs font-medium text-[var(--text-primary)]">{item.dimension}</div>
              {item.note && (
