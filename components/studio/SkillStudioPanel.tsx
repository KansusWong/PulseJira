    ...
    return (
      <div>
        <button
          ...
          className="w-full flex items-center gap-1.5 px-2 py-1 text-xs text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-elevated)] rounded transition-colors"
...
export function SkillStudioPanel() {
  ...
  if (tabs.length === 0) {
    return (
      <div className="flex flex-col h-full rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-glass)] backdrop-blur-[var(--glass-blur)]">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="text-xs text-[var(--text-muted)]">{t("studio.noTabs")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-glass)] backdrop-blur-[var(--glass-blur)] overflow-hidden">
      {/* ── Top bar ── */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border-subtle)]">
        <div className="flex items-center gap-2 text-sm font-medium text-[var(--text-primary)]">
          <Wrench className="w-4 h-4" />
          <span>{t("studio.title")}</span>
          <span className="text-[10px] text-[var(--text-muted)] font-mono">{tabs.length}</span>
        </div>
        <button
          onClick={hidePanel}
          className="p-1 text-[var(--text-muted)] hover:text-[var(--text-primary)] rounded transition-colors"
      ...
      {/* ── Tab bar ── */}
      <div className="flex items-center gap-0 px-2 py-1 border-b border-[var(--border-subtle)] overflow-x-auto">
        {tabs.map((tab) => (
          <div
            key={tab.skillId}
            className={clsx(
          ...
          >
            {renaming && activeTabId === tab.skillId ? (
              <input
                ...
                className="bg-transparent border border-[var(--border-subtle)] rounded px-1 py-0 text-xs text-[var(--text-primary)] outline-none w-full"
            ...
            <button
              ...
              className="flex-shrink-0 p-0.5 rounded opacity-0 group-hover/tab:opacity-100 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-opacity"
            >
              <X className="w-3 h-3" />
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="flex-1 flex min-h-0">
          {/* Left: File tree */}
          <div className="w-[160px] flex-shrink-0 border-r border-[var(--border-subtle)] overflow-y-auto py-2">
            <FileTreeView
              nodes={fileTree}
            ...
            <div className="flex items-center justify-between px-3 py-1.5 border-b border-[var(--border-subtle)]">
              ...
              {fileContent !== null && (
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-0.5 text-[11px] text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-elevated)] rounded transition-colors flex-shrink-0"
